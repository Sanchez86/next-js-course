...
export const Rating = ({
  ...
  const hangeOnEnter = (i: number, e: KeyboardEvent<SVGElement>) => {
  ...
  const constructRating = (currentRating: number) => {
    const updatedArray = ratingArray.map((el: JSX.Element, i: number) => {
      return (
        <svg
          onMouseEnter={() => hengeDisplay(i + 1)}
          onMouseLeave={() => hengeDisplay(rating)}
          onClick={() => hengeRating(i + 1)}
          tabIndex={isEditable ? 0 : -1}
          onKeyDown={(e: KeyboardEvent<SVGElement>) =>
            isEditable && hangeOnEnter(i + 1, e)
          }
